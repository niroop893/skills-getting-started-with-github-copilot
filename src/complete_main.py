...
def screenshot_chat():
    try:
        ...
        # Select area on Pearson VUE browser
        x, y, width, height = select_area()
        
        if width < 10 or height < 10:
            show_bot_window(bot_hwnd)  # Restore bot window
            return jsonify({"reply": "⚠️ Selected area too small"})
        
        # Take screenshot of Pearson VUE browser area
        screenshot = pyautogui.screenshot(region=(x, y, width, height))
        
        # Restore bot window immediately
        ...
        # Ensure bot window is restored even if error occurs
        try:
            show_bot_window(bot_hwnd)
        except: pass
        print(f"Error: {e}")
